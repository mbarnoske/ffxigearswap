-- RDM TODO List (updated 10/4/2019)

-- Weapons : Seq R15, Excalibur?, Raetic Staff +1?
-- TP : New Malignance Set
-- WS : Mache +1 Earring (CDC, Evis), Lilith Club
-- Enfeeb : Regal Cuffs, Lilith Ear?
-- Enhancing : +10 Enh Dur Telchine Head (w/ Duskdim, currently 9%)
-- Nuking : Amalric Nails +1?, Lilith Ear?
-- Healing: Needs work - Enmity-, Healing Magic Skill, Cure Pot II  (check Nuance FFXIAH node 349)

-- Stymie/ES should be casting in max enfeeb duration
...
-- CroceaC / Sanguine (No Magic Buff Fully Unresisted ~22-23k dps) : ~5500+ estimated, spreadsheet doesn't work with Sanguine
-- Naegling / Savage ???
...
-- Tanking Gear Set :
  -- Theoretical Enmity + for RDM Set TODO...
    ...
    -- Theoretically Possible: +93 ENM (+87 if not swapping weapon, +86 not including rare waist)
...
function user_setup()
   ...
   state.OffenseMode:options('MaxHaste', 'MaxHasteCroceaC', 'MaxHasteSB', 'MaxHasteCroceaCSB', 'MaxHasteMidAcc', 'MaxHasteMidAccCroceaC',
                             'MaxHasteHiAcc', 'MaxHasteMaxAcc', 'LowHaste', 'Tank', 'TankAcc', 'TH', 'NoTP')
   ...
   INTAccuracySpells = S{'Sleep', 'Sleep II', 'Sleepga', 'Sleepga II', 'Break', 'Breakga', 'Blind', 'Blind II', 'Bind', 'Dispel', 'Gravity', 'Gravity II'}
...
function init_gear_sets()
   ...
   sets.precast.WS['Savage Blade'] = {range="Ullr",
      ...
      body="Viti. Tabard +3",hands="Atrophy Gloves +3",ring1="Shukuyu Ring",ring2="Epaminondas's Ring",
   ...
   sets.precast.WS['Aeolian Edge'] = {ammo="Ghastly Tathlum +1",
      ...
      back=SucellosINT,waist="Orpheus's Sash",legs="Amalric Slops +1",feet="Vitiation Boots +3"}
					
   sets.precast.WS['Aeolian Edge'].TH = {ammo="Ghastly Tathlum +1",
      head="Merlinic Hood",neck="Sanctity Necklace",ear1="Regal Earring",ear2="Friomisi Earring",
      body="Amalric Doublet +1",hands=ChironicTH,ring1="Freke Ring",ring2="Shiva Ring +1",
      back=SucellosINT,waist="Chaac Belt",legs="Amalric Slops +1",feet="Vitiation Boots +3"}			
   ...
   -- Self uses: Gear 2: Dls Torque +2 25%, Ghostfyre Cape 20%, Telchine Head/Legs 9%, 10%
   ...
   -- Others uses: Gear 2: Dls Torque +2 25%, Ghostfyre Cape 20%
   -- Haste Self = (180 + 20) * (1 + 0.15 + 0.3 + 0.2 + 0.1) * (1 + 0.25 + 0.2 + 0.19) ~= 574s (9:34) TESTED: 9:34
   -- Haste Comp Self = (180 + 20) * (1 + 0.15 + 0.3 + 0.2 + 0.1) * (1 + 0.25 + 0.2 + 0.19) * (3.0) ~= 1701 (28:21) TESTED: 28:22
   -- Haste Comp Others = (180 + 20) * (1 + 0.35) * (1 + 0.3 + 0.2 + 0.1) * (1 + 0.25 + 0.2) ~= 626.4s (10:26) TESTED: 10:26
   ...
   sets.midcast['Diaga'] = set_combine(sets.midcast.EnfeebMNDPotency, {hands=ChironicTH,waist="Chaac Belt",feet="Volte Boots"})
   ...
   sets.midcast.INTPotency = {main=GrioEnfeeb,sub="Enki Strap",ammo="Regal Gem",
      head="Viti. Chapeau +3",neck="Dls. Torque +2",ear1="Digni. Earring",ear2="Regal Earring",
      body="Atrophy Tabard +3",hands="Kaykaus Cuffs +1",ring1="Freke Ring",ring2="Shiva Ring +1",
   ...
   -- TODO : Consider duration pieces for Sleep and Break
   ...
   -- TODO : Enfeebling Acc and Duration set for Sleep, Bind, Break, etc
   ...
   -- Jhakri +2 : 7%/0 (INT33 MAcc42 MAtt39)
   -- Total : 39%/35%, Cap 40%/?%
   ...
   -- Idle sets
   --sets.idle.Town = {main="Crocea Mors",sub="Tauret",range="Ullr",
   ...
   --   back=SucellosDW,waist="Orpheus's Sash",legs="Volte Tights",feet="Carmine Greaves +1"}
   sets.idle.Town = {main=GrioEnfeeb,sub="Mephitis Grip",ammo="Regal Gem",
      head="Viti. Chapeau +3",neck="Dls. Torque +2",ear1="Digni. Earring",ear2="Regal Earring",
      body="Lethargy Sayon +1",hands="Kaykaus Cuffs +1",ring1="Stikini Ring +1",ring2="Stikini Ring +1",
      back=SucellosMND,waist="Rumination Sash",legs="Chironic Hose",feet="Vitiation Boots +3"}
   ...
   sets.idle.Normal = {main="Bolelabunga",sub="Beatific Shield +1",ammo="Homiliary",
      head="Viti. Chapeau +3",neck="Loricate Torque +1",ear1="Etiolation Earring",ear2="Loquac. Earring",
      body="Shamash Robe",hands=ChironicRefreshHands,ring1="Stikini Ring +1",ring2="Stikini Ring +1",
      back=SucellosSTP,waist="Flume Belt",legs="Carmine Cuisses +1",feet=MerlinicRefreshFeet}
                         
   -- "idle.DT"  A set for aggroing/tanking unengaged (Assuming ShellV 24%) 
      -- Main Naegling                                          [Mafic Cudgel? (10% PDT)]
   ...
   -- Base TP set (unused)
   sets.engaged = {main="Naegling",sub="Beatific Shield +1",range="Ullr",
      head=TaeonHeadTP,neck="Anu Torque",ear1="Sherida Earring",ear2="Telos Earring",
   ...
   sets.engaged.DW = set_combine(sets.engaged, {main="Naegling",sub="Tauret",back=SucellosDW})

   -- MaxHaste : A Max DPS TP set (Single and Dual Wield) for the Naegling, Sequence, Almace, and Murgleis  [1204 Acc with Composure & R15Sequence/Tauret]
   sets.engaged.MaxHaste = {main="Naegling",sub="Beatific Shield +1",range="Ullr",
      head=TaeonHeadTP,neck="Anu Torque",ear1="Sherida Earring",ear2="Telos Earring",
      body="Ayanmo Corazza +2",hands=TaeonHandsTP,ring1="Chirich Ring +1",ring2="Chirich Ring +1",
      back=SucellosSTP,waist="Windbuffet Belt +1",legs=TaeonLegsTP,feet="Carmine Greaves +1"}
   sets.engaged.DW.MaxHaste = set_combine(sets.engaged.MaxHaste, {main="Naegling",sub="Tauret",back=SucellosDW})

   -- MaxHasteCroceaC : A Max DPS TP set (Single and Dual Wield) for the Crocea Mors(C)  [1241 Acc with Composure & Crocea/Tauret]
   sets.engaged.MaxHasteCroceaC = {main="Crocea Mors",sub="Beatific Shield +1",range="Ullr",
      head=TaeonHeadTP,neck="Anu Torque",ear1="Sherida Earring",ear2="Telos Earring",
      body="Volte Harness",hands="Aya. Manopolas +2",ring1="Hetairoi Ring",ring2="Ilabrat Ring",
      back=SucellosSTP,waist="Orpheus's Sash",legs=TaeonLegsTP,feet="Carmine Greaves +1"}
   sets.engaged.DW.MaxHasteCroceaC = set_combine(sets.engaged.MaxHasteCroceaC, {main="Crocea Mors",sub="Tauret",back=SucellosDW})
            
   -- MaxHasteSB : A subtle blow TP set (Single and Dual Wield) for the Naegling, Sequence, Almace, and Murgleis  [1203 Acc with Composure & R15Sequence/Tauret]
   sets.engaged.MaxHasteSB = {main="Naegling",sub="Beatific Shield +1",range="Ullr",
      head=TaeonHeadTP,neck="Anu Torque",ear1="Sherida Earring",ear2="Telos Earring",
      body="Volte Harness",hands=TaeonHandsTP,ring1="Chirich Ring +1",ring2="Chirich Ring +1",
      back=SucellosSTP,waist="Windbuffet Belt +1",legs="Volte Tights",feet="Carmine Greaves +1"}
   sets.engaged.DW.MaxHasteSB = set_combine(sets.engaged.MaxHasteSB, {main="Naegling",sub="Tauret",back=SucellosDW})

   -- MaxHasteCroceaCSB : A subtle blow TP set (Single and Dual Wield) for the Crocea Mors(C)  [1254 Acc with Composure & Crocea/Tauret]
   sets.engaged.MaxHasteCroceaCSB = {main="Crocea Mors",sub="Beatific Shield +1",range="Ullr",
      head=TaeonHeadTP,neck="Anu Torque",ear1="Sherida Earring",ear2="Telos Earring",
      body="Volte Harness",hands="Aya. Manopolas +2",ring1="Chirich Ring +1",ring2="Chirich Ring +1",
      back=SucellosSTP,waist="Orpheus's Sash",legs="Volte Tights",feet="Carmine Greaves +1"}
   sets.engaged.DW.MaxHasteCroceaCSB = set_combine(sets.engaged.MaxHasteCroceaCSB, {main="Crocea Mors",sub="Tauret",back=SucellosDW})
            
   -- MaxHasteMidAcc : A more accurate TP set (Single and Dual Wield) [1302 Acc with Composure & R15Sequence/Tauret]
   sets.engaged.MaxHasteMidAcc = {main="Naegling",sub="Beatific Shield +1",range="Ullr",
      head="Aya. Zucchetto +2",neck="Anu Torque",ear1="Sherida Earring",ear2="Telos Earring",
      body="Volte Harness",hands="Atrophy Gloves +3",ring1="Chirich Ring +1",ring2="Ilabrat Ring",
      back=SucellosSTP,waist="Kentarch Belt +1",legs="Carmine Cuisses +1",feet="Carmine Greaves +1"}
   sets.engaged.DW.MaxHasteMidAcc = set_combine(sets.engaged.MaxHasteMidAcc, {main="Naegling",sub="Tauret",ear2="Suppanomimi"})
            
   -- MaxHasteMidAccCroceaC : A more accurate TP set (Single and Dual Wield) for the Crocea Mors(C) [1298 Acc with Composure & Crocea/Tauret]
   sets.engaged.MaxHasteMidAccCroceaC = {main="Crocea Mors",sub="Beatific Shield +1",range="Ullr",
      head="Aya. Zucchetto +2",neck="Anu Torque",ear1="Sherida Earring",ear2="Telos Earring",
      body="Volte Harness",hands="Aya. Manopolas +2",ring1="Hetairoi Ring",ring2="Chirich Ring +1",
      back=SucellosSTP,waist="Orpheus's Sash",legs="Carmine Cuisses +1",feet="Carmine Greaves +1"}
   sets.engaged.DW.MaxHasteMidAccCroceaC = set_combine(sets.engaged.MaxHasteMidAccCroceaC, {main="Crocea Mors",sub="Tauret",ear2="Suppanomimi"})
            
   -- MaxHasteHiAcc : A TP set with all reasonable Acc gear (Single and Dual Wield) [1364 Acc with Composure & Crocea/Tauret]
   sets.engaged.MaxHasteHiAcc = {main="Naegling",sub="Beatific Shield +1",range="Ullr",
      head="Carmine Mask +1",neck="Combatant's Torque",ear1="Telos Earring",ear2="Digni. Earring",
      body="Ayanmo Corazza +2",hands="Atrophy Gloves +3",ring1="Chirich Ring +1",ring2="Chirich Ring +1",
      back=SucellosSTP,waist="Kentarch Belt +1",legs="Carmine Cuisses +1",feet="Carmine Greaves +1"}
   sets.engaged.DW.MaxHasteHiAcc = set_combine(sets.engaged.MaxHasteHiAcc, {main="Naegling",sub="Tauret",ear2="Suppanomimi"})
            
   -- MaxHasteMaxAcc : A TP set with every Acc gear, no matter how bad (Single and Dual Wield) [1422 Acc with Composure]
   sets.engaged.MaxHasteMaxAcc = {main="Naegling",sub="Beatific Shield +1",range="Ullr",
   ...
   sets.engaged.DW.MaxHasteMaxAcc = set_combine(sets.engaged.MaxHasteMaxAcc, {main="Naegling",sub="Tauret"})
            
   -- TH : A TP set with TH+4 (Single and Dual Wield) 
   sets.engaged.TH = {main="Naegling",sub="Beatific Shield +1",range="Ullr",
      head=TaeonHeadTP,neck="Anu Torque",ear1="Sherida Earring",ear2="Telos Earring",
      body="Volte Harness",hands=ChironicTH,ring1="Hetairoi Ring",ring2="Ilabrat Ring",
      back=SucellosSTP,waist="Chaac Belt",legs="Volte Tights",feet="Volte Boots"}
   sets.engaged.DW.TH = set_combine(sets.engaged.TH, {main="Naegling",sub="Tauret",back=SucellosDW})

   -- LowHaste : A TP set for when not haste capped, adding DW+ gears (Single and Dual Wield)
   sets.engaged.LowHaste = {main="Naegling",sub="Beatific Shield +1",range="Ullr",
      head=TaeonHeadTP,neck="Anu Torque",ear1="Sherida Earring",ear2="Telos Earring",
      body="Volte Harness",hands=TaeonHandsTP,ring1="Hetairoi Ring",ring2="Ilabrat Ring",
      back=SucellosSTP,waist="Windbuffet Belt +1",legs="Volte Tights",feet="Carmine Greaves +1"}
   sets.engaged.DW.LowHaste = set_combine(sets.engaged.LowHaste, {main="Naegling",sub="Tauret",ear1="Sherida Earring",ear2="Suppanomimi",back=SucellosDW,waist="Reiki Yotai",feet=TaeonFeetTP})

   -- Tank : An engaged set for tanking /BLU (TODO - not completed)
   sets.engaged.Tank = {main="Naegling",sub="Beatific Shield +1",ammo="Staunch Tathlum",
   ...
   sets.engaged.TankAcc = {main="Naegling",sub="Beatific Shield +1",ammo="Staunch Tathlum",
      head="Aya. Zucchetto +2",neck="Loricate Torque +1",ear1="Sherida Earring",ear2="Telos Earring",
      body="Ayanmo Corazza +2",hands="Aya. Manopolas +2",ring1="Defending Ring",ring2="Gelatinous Ring +1",
      back=SucellosSTP,waist="Kentarch Belt +1",legs="Carmine Cuisses +1",feet="Carmine Greaves +1"}
   sets.engaged.DW.TankAcc = sets.engaged.TankAcc

   -- Working setup for Ambu Aug2019 Frog N
   sets.engaged.NoTP = {main="Ceremonial Dagger",sub="Beatific Shield +1",ammo="Staunch Tathlum",
      head="Chironic Hat",neck="Loricate Torque +1",ear1="Dedition Earring",ear2="Suppanomimi",
      body="Volte Harness",hands="Aya. Manopolas +2",ring1="Chirich Ring +1",ring2="Chirich Ring +1",
      back=SucellosSTP,waist="Orpheus's Sash",legs=TaeonLegsTP,feet="Carmine Greaves +1"}
   ...
   --sets.engaged.NoTP = {main="Ceremonial Dagger",sub="Beatific Shield +1",ammo="Staunch Tathlum",
   --   head="Chironic Hat",neck="Combatant's Torque",ear1="Sherida Earring",ear2="Suppanomimi",
   --   body="Volte Harness",hands="Aya. Manopolas +2",ring1="Chirich Ring +1",ring2="Chirich Ring +1",
   --   back=SucellosSTP,waist="Orpheus's Sash",legs=TaeonLegsTP,feet="Carmine Greaves +1"}
   --sets.engaged.DW.NoTP = set_combine(sets.engaged.NoTP, {sub="Ceremonial Dagger",ear2="Suppanomimi",back=SucellosDW})
   
   sets.TempGear = {}
   --sets.FrontlineWeapons = {main="Crocea Mors",sub="Tauret",range="Ullr"}
   --sets.FrontlineWeapons = {main="Naegling",sub="Tauret",range="Ullr"}
   sets.FrontlineWeapons = {main="Ceremonial Dagger",sub="Ceremonial Dagger",ammo="Jukukik Feather"}
   sets.BacklineWeapons = {main="Bolelabunga",sub="Beatific Shield +1",ammo="Homiliary"}
end
      ...
      if spell.target.type == 'SELF' then
         ...
         if EnhancingPotencySpells:contains(spell.english) then
            if state.CombatMode.value == 'Frontline' then
               ...
               send_command('gs enable main sub range ammo')
            end
   ...
   if spell.target.type == 'SELF' then
      if EnhancingPotencySpells:contains(spell.english) or PhalanxSpells:contains(spell.english) then
         if state.CombatMode.value == 'Frontline' then
            ...
            equip(sets.TempGear)
            send_command('gs disable main sub range ammo')
